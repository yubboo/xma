...
- Windows `[1]` 必须把 Bun/OpenTUI/Solid/@types-bun 作为 pnpm Workspace JavaScript Runtime 统一管理，并在项目根直接执行标准 `pnpm install`；只有 `[8] 刷新 · JavaScript Runtime` 主动执行 latest 更新。`[1]` 同时必须确保 Git、Node.js、兼容 pnpm、Rust/Cargo、rustfmt、MSVC 与 Cargo crates 完整可用：缺失或不满足项目硬要求时自动安装/修正，已满足要求就复用，不为追新强制升级。`[4]`、`[7]`、`build:cli` 只能消费已安装版本并真实探针，禁止隐式联网。Bun/OpenTUI 不得再拥有 `xma-path/bun|opentui`、`XMA_BUN_HOME`、独立下载器或 checkout Bun state；`[1]` 缺失 Rust 时使用 checkout 默认 `xma-path`，`[9]` 可单独修复并选择其他盘符。
- Windows JavaScript Runtime 的网络访问统一由 pnpm/npm registry 负责；禁止恢复 Bun ZIP/tgz/SourceForge/独立 checksum 下载器。`[1]` 不得替用户测速或强制切换 npm registry，直接尊重 pnpm/用户当前配置；`[8]` 也默认沿用当前 registry。Rustup 仍可在 Rust 官方与 RsProxy 间切换，并且 `RUSTUP_DIST_SERVER/RUSTUP_UPDATE_ROOT` 只在当前 XMA 进程内生效。
...
- Windows `[4]/[7]` 的 JavaScript Runtime 只从 Workspace `node_modules` 读取，不做盘符扫描；Rust/Cargo 在 checkout state 缺失但外部 `xma-path/rust` 实体仍存在时允许离线真实再发现。盘符扫描只属于 Rust 解析入口，普通 `xma-dev.bat` 菜单启动不得自动扫盘。
