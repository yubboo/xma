...
- Windows `xma-dev.bat → [1]` 可以注册开发态 `xiaoyu / xma`，但只允许把 checkout 本地控制状态 `.git/xma-state/dev-bin`（非 Git 树回退 `.cache/xma-state/dev-bin`）加到当前用户 User PATH；禁止把整个仓库或维护脚本目录加入 PATH，禁止写 Machine PATH。
...
- Windows `[1]` 的 Bun/OpenTUI 与 Rust/Cargo 必须先真实检测后再决定是否安装：缺失时分别询问 Y/N，选择 N 只跳过对应组件并继续；主菜单 `[8]` / `[9]` 提供单独补装。安装位置统一按依赖根选择：`[1] <当前 checkout>/xma-path`（默认/推荐）、`[2] D:/xma-path`、`[3] 用户输入真实盘符后使用 <盘符>:/xma-path`。项目默认 Bun/OpenTUI/Rust 分别位于 `xma-path/bun`、`xma-path/opentui`、`xma-path/rust`；checkout 控制状态与开发 shim 位于 `.git/xma-state`（非 Git 树回退 `.cache/xma-state`）。如果用户选择 `[2] D:/xma-path` 或 `[3]` 外部盘符，项目根不得仅为 state/dev-bin 再生成第二个 `xma-path`；`[4]`、`[7]`、`build:cli` 必须从 checkout 状态恢复同一真实依赖位置并重新校验。旧 `.xma` 只允许作为一次迁移源，不得继续成为现役状态目录。 安装位置交互必须在 `[1]/[2]/[3]` 三行本身原位高亮，支持 `↑/↓` 移动、`Enter` 确认与数字键直达，禁止另起“当前选择”状态行。Rust 准备禁止 `rustup override set stable` 这类目录级 absolute-path 绑定；自管 Rust 只通过隔离的 `RUSTUP_HOME/CARGO_HOME` 和 stable default 恢复。
- Windows `[1]/[8]/[9]` 的网络准备必须可见、可切换且不把镜像设置永久污染用户环境：默认 `XMA_DOWNLOAD_SOURCE=auto` 对官方源与已批准镜像做短探针，优先更快者；连接停滞/失败自动切换备用源。Bun Runtime 使用官方 GitHub + SourceForge exact mirror，并以固定版本官方 GitHub Release asset digest（内置 SHA-256）校验 ZIP，禁止为了校验再引入第二次 `SHASUMS256.txt` 网络依赖；Windows Schannel 仅允许 `--ssl-revoke-best-effort` 处理吊销服务离线，不能关闭 TLS 证书链验证；OpenTUI 的 Bun package install 可在 npm 官方与 `registry.npmmirror.com` 间自动选择；Rustup 可在 Rust 官方与 RsProxy 间自动选择，并且 `RUSTUP_DIST_SERVER/RUSTUP_UPDATE_ROOT` 只在当前 XMA 进程内生效。`curl.exe` 可用时下载必须显示实时 progress bar，并对持续低速做 stall fallback；Rust/Bun 子安装进程必须继承真实终端并显示原生日志/进度。用户可显式设置 `XMA_DOWNLOAD_SOURCE=official|mirror` 覆盖自动模式。
