...
- Windows `[1]` 必须把 Bun/OpenTUI/Solid/@types-bun 作为 pnpm Workspace JavaScript Runtime 统一管理：依赖声明使用 registry `latest`，只有 `[1]` 与 `[8] 刷新 · JavaScript Runtime` 主动执行 latest 更新并刷新 lockfile/node_modules；`[4]`、`[7]`、`build:cli` 只能消费已安装版本并真实探针，禁止隐式联网。Bun/OpenTUI 不得再拥有 `xma-path/bun|opentui`、`XMA_BUN_HOME`、独立下载器或 checkout Bun state。Rust/Cargo 继续独立：缺失时 `[1]` 询问 Y/N，`[9]` 单独补装；依赖根只用于 Rust，安装位置菜单支持 `↑/↓`、Enter 与数字直选；Rust 禁止 `rustup override set stable`，只使用隔离 `RUSTUP_HOME/CARGO_HOME` 与 stable default。
- Windows JavaScript Runtime 的网络访问统一由 pnpm/npm registry 负责；禁止恢复 Bun ZIP/tgz/SourceForge/独立 checksum 下载器。`[1]/[8]` 可在 npm 官方与批准镜像间切换 registry，但设置只能作用于当前准备进程。Rustup 仍可在 Rust 官方与 RsProxy 间切换，并且 `RUSTUP_DIST_SERVER/RUSTUP_UPDATE_ROOT` 只在当前 XMA 进程内生效。
