...
- **Terminal 快捷提示按需化。** `Ctrl+C` 属于上下文动作，不得长期占用 PromptDock 固定快捷栏；只有用户实际按下时才通过 transient notice 显示“已复制 / 已请求中止 / 已取消或拒绝 / 再按一次退出”。固定快捷栏只保留长期可发现的模式、命令、搜索、快捷命令、返回等入口，并优先维持单行。Prompt 的 slash command 必须复用 **Ctrl+P/Ctrl+K 同一个居中 `ListDialog` 命令面板**：单独输入 `/` 只保留在 Prompt，不展开整张命令表；只有出现首个命令字母后才打开同款居中 modal，并把当前 `/<prefix>` 预填到搜索框，以 canonical shortcut prefix 过滤，例如 `/h` 只显示 `/h...`、继续输入 `/he` 继续收窄。slash 候选的 shortcut 必须把**已输入匹配前缀**与**剩余未输入字符**分色渲染（例如 query `/he` 时 `/help` 中 `/he` 为强调色、`lp` 为弱化色），不能整串同色让用户无法确认匹配位置。slash modal 与 Ctrl+P/Ctrl+K 必须共享 `commandPaletteOptions()`、同一 `ListDialog`、同一 `runCommand()` 与 ↑/↓/Enter/Esc 焦点合同；可搜索 Dialog settle 前必须 blur 自己的 Textarea，父 Prompt 只能在 modal state 卸载后的下一 microtask 且确认没有新 Dialog/SetupFlow 时 refocus，禁止两个 Textarea 抢焦点。禁止 PromptDock 再绘制 inline candidate list、ghost suffix 或依赖 `Ctrl+Space` 补全。Tab/Shift+Tab 在主 Prompt 永远保留给 Build/Plan/Compose 模式切换。`/help` 与所有 slash commands 继续从同一个 canonical command catalog 派生，禁止第二份 slash-only 假命令表、legacy-only suggestion 或只显示不能执行的项目。`/help` 必须持久说明全部命令及用途，而不是只抛一串名称。Prompt caret 必须继续由 OpenTUI Textarea 原生拥有；用户已确认主 Prompt 的视觉基线为原生浅色 block caret + native blinking。若 Terminal/OpenTUI 不提供 per-app blink interval，只允许使用原生 cursor shape/blink on-off，禁止恢复应用层定时 `showCursor` 模拟慢闪烁；修 slash、Dialog、Metrics 等其它模块不得顺手改变已验收的 caret 形态。
