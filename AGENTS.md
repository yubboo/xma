...
- **Terminal Session Metrics 分层投影。** #05 canonical `session/metrics` 是唯一数据源。Prompt 主状态行保持 Mode 左对齐、右侧按 `context + account/quota` 摘要与 `Provider + Model + Ready + Reasoning` 两组相邻右对齐，常见宽度目标形如 `上下文 0.2% · 1,793/1.0m · 余额 ¥12.02   ● Provider · model · 模型已就绪 · high`。下方 detail row 不得重复 model/context/balance，只显示 billing/tokens/真实费用/permission 等次要信息；空间不足必须按优先级裁字段，禁止自动换行挤高 PromptDock。
