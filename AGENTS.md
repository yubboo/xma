...
- Web / CLI 在 `[1]` 成功后不得再次执行 `pnpm install`、`pnpm rebuild esbuild` 或其他重复依赖安装。`[4] Xiaoyu Terminal` 启动前必须对 Native Rust 源码、Cargo 配置/lockfile 与 rustc 版本形成指纹：只有指纹变化或 Native 可执行文件缺失时才执行 `cargo build --package xma-native-runtime --offline`；指纹一致时直接复用 `.cache/cargo-target/` 构建产物并秒级进入 Trust/TUI。每次启动仍把构建结果复制到 `.cache/native-runtime/runs/` 的唯一 staging exe，禁止直接运行/覆盖 Cargo target 中可能被旧进程锁定的 exe。该流程只使用 `[1]` 已预取 crates，不允许偷偷联网下载。
