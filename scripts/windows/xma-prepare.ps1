<#
文件作用：XMA Windows 一键开发环境准备器，供开发控制台、GitHub Helper、发布脚本共同复用。
关联模块：xma-console.ps1、xma-github.ps1、xma-build-release.ps1、package.json、Cargo.toml。
当前实现：检查/安装 Git、Node.js 22+、pnpm、Rustup、MSVC Build Tools，并安装 TS/Electron/Rust 项目依赖。
职责边界：系统级安装必须经过用户 Y/N；项目级 pnpm install / cargo fetch 可以自动执行。
#>
...
function Ensure-XmaPackage([string]$Command, [string]$PackageId, [string]$DisplayName) {
  if (Get-Command $Command -ErrorAction SilentlyContinue) {
    Write-Host "[OK] $DisplayName" -ForegroundColor Green
    return
  }
  Write-Host "[缺少] $DisplayName" -ForegroundColor Yellow
  if (-not (Confirm-XmaAction "是否自动安装 $DisplayName？")) { throw "$DisplayName 是 XMA 开发环境的必要依赖。" }
  Ensure-XmaWinget
  Invoke-XmaExternal -FilePath 'winget.exe' -ArgumentList @('install','--id',$PackageId,'--exact','--accept-source-agreements','--accept-package-agreements')
  Refresh-XmaPath
  if (-not (Get-Command $Command -ErrorAction SilentlyContinue)) { throw "$DisplayName 安装后仍未出现在 PATH，请重新打开终端后再运行。" }
}

function Ensure-XmaMsvcBuildTools {
  ...
  Write-Host '[缺少] Visual Studio C++ Build Tools（Rust MSVC 链接器需要）' -ForegroundColor Yellow
  if (-not (Confirm-XmaAction '是否自动安装 Visual Studio 2022 Build Tools + C++ Toolchain？')) {
    throw 'Windows Rust Native 构建需要 MSVC C++ Build Tools。'
  }
  Ensure-XmaWinget
...
Write-Host '  XMA 一键准备环境' -ForegroundColor Cyan
Write-Host '====================================================================' -ForegroundColor DarkCyan
Write-Host '说明：XMA 会自动检查环境；缺少系统级依赖时才询问 Y/N。' -ForegroundColor DarkGray
Write-Host ''
...
if (-not $hasCargo -or -not $hasRustc) {
  ...
  if (-not $hasRustup) { throw 'rustup 安装后仍未出现在 PATH，请重新打开终端后再运行 XMA。' }
  Write-Host '[安装] 正在安装 Rust stable toolchain（minimal profile）...' -ForegroundColor Yellow
...
if (-not (Get-Command cargo.exe -ErrorAction SilentlyContinue)) { throw 'Rust stable 已处理，但 cargo.exe 仍不可用。请重新打开终端后再运行 XMA。' }
if (-not (Get-Command rustc.exe -ErrorAction SilentlyContinue)) { throw 'Rust stable 已处理，但 rustc.exe 仍不可用。请重新打开终端后再运行 XMA。' }
...
Write-Host ''
Write-Host '[6/7] TypeScript / Electron / 项目依赖' -ForegroundColor Cyan
Write-Host '[检查] 正在检查 pnpm Workspace 依赖状态...' -ForegroundColor DarkCyan
Write-Host '[安全] pnpm 11 安装脚本采用项目白名单：仅允许 electron、electron-winstaller、esbuild 执行安装脚本。' -ForegroundColor DarkYellow
Write-Host '[安装] 正在安装/校验 TypeScript、Electron、Vite、构建工具及所有 Workspace 依赖...' -ForegroundColor Yellow
Invoke-XmaExternal -FilePath 'pnpm.cmd' -ArgumentList @('install')
Write-Host '[修复] 正在确保已批准依赖的安装脚本完整执行...' -ForegroundColor DarkCyan
...
Write-Host '[验证] 正在验证 esbuild Native Binary...' -ForegroundColor DarkCyan
Invoke-XmaExternal -FilePath 'pnpm.cmd' -ArgumentList @('exec','esbuild','--version')
Write-Host '[通过] TypeScript / Electron / Workspace 依赖已准备完成。' -ForegroundColor Green

Write-Host ''
Write-Host '[7/7] Rust 依赖预取' -ForegroundColor Cyan
Write-Host '[同步] 正在解析并预取 XMA Rust crates...' -ForegroundColor DarkCyan
Invoke-XmaExternal -FilePath 'cargo.exe' -ArgumentList @('fetch')
Write-Host '[通过] Rust crates 已准备完成。' -ForegroundColor Green

Write-Host ''
Write-Host '====================================================================' -ForegroundColor DarkCyan
Write-Host '[完成] XMA 开发环境准备完成。' -ForegroundColor Green
Write-Host 'Git / Node.js / pnpm / Rust / MSVC / TypeScript / Electron 依赖均已检查。' -ForegroundColor Cyan
Write-Host '====================================================================' -ForegroundColor DarkCyan
