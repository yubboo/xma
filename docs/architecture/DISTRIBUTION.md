...
- 主 Prompt 使用 OpenTUI 原生 `TextareaRenderable` / `<textarea>` 管理 IME、选择、粘贴与多行输入；Active Renderer 禁止再次输出 `CURSOR_MARKER`、手写 DECTCEM/mouse-reporting 或 Pi TUI reverse-video 光标补丁。OpenTUI 工作台中的 terminal hardware cursor 必须保持隐藏，Textarea 只负责逻辑 focus/编辑，避免 Windows Terminal Text Cursor Indicator 被星星/流星等动画 dirty frame 搬到背景 cell。Tab 模式切换、Ctrl+P/Ctrl+K、Esc 返回和 Dialog focus 必须统一走 OpenTUI 键盘/焦点体系。
