...
- 主 Prompt 使用 OpenTUI 原生 `TextareaRenderable` / `<textarea>` 管理 IME、选择、粘贴与多行输入；Active Renderer 禁止再次输出 `CURSOR_MARKER`、手写 DECTCEM/mouse-reporting 或 Pi TUI reverse-video 光标补丁。OpenTUI 工作台中的 terminal cursor 必须由 focused Textarea 原生拥有：Textarea 的 `renderCursor()` 负责真实 caret/IME 坐标，星星/流星等动画 dirty frame 只能让 active editor `requestRender()` 重新提交 cursor；禁止应用层定时把 cursor 隐藏/搬到 `(0,0)`。Tab 模式切换、Ctrl+P/Ctrl+K、Esc 返回和 Dialog focus 必须统一走 OpenTUI 键盘/焦点体系。
- Home / Transcript / Prompt / Shortcut / Notice 使用 OpenTUI Flexbox 响应式布局，左右边距必须对称；Prompt 的真实输入光标必须位于输入组件内部，禁止退回“静态卡片 + 底部 readline”伪 TUI；Transcript 中用户消息固定右对齐，Xiaoyu thinking / activity / answer 固定左对齐；每轮从用户提交开始立即提供默认折叠且实时刷新的 `思考了 Nh Nm Ns` 活动项，运行中按真实墙钟时间递增，完成/中止/失败后冻结最终耗时；运行中和完成后均可展开，展开只展示截至当前真实发生的公开 Runtime 状态与 Tool Call/Result 摘要，禁止展示原始 reasoning 正文或 Secret；
...
- **首次模型配置**：工作区信任通过后，如果当前没有已配置 Brain/Profile，必须在**同一 Xiaoyu TUI 的居中 modal**自动打开“模型 / 提供方”配置；一旦 Profile 已配置，后续启动跳过此第二步。`Ctrl+P → 模型 / 提供方` 永久保留，并与首次 Setup 复用同一 Provider/API Key/Model/Reasoning 实现；连接测试/Brain Ready Probe 只作为可选诊断；
...
- Terminal 已提供用户级 `brain.json` Provider Profile：可通过 `Ctrl+P → Brain / Provider` 从真实 Provider Catalog 添加 DeepSeek Official 或自定义 OpenAI-compatible Profile；同一品牌可保存多个账号/Profile。品牌 Provider 首次配置主路径固定为 API Key → 真实远程模型选择 → 推理强度 → 已就绪；连接测试是可选诊断；API Key 默认写入 OS Credentials，环境变量引用继续作为兼容路径，`/model` 可再次切换真实模型，Reasoning effort 作为非 Secret Profile option 持久化。`brain.json` v3 保存真实 Provider/Profile/Adapter/Model 与 Credential Reference，永远不保存 Secret 值，也不写入 Session；v1 `credentialEnv`、v2 generic Profile 与 `XIAOYU_BASE_URL / XIAOYU_MODEL / XIAOYU_API_KEY` 继续兼容迁移读取；
