...
- Terminal Active Renderer 已迁移为 pnpm Workspace 管理的 `Bun + OpenTUI + Solid`；`[1]` 只安装当前 Workspace 依赖，`[8]` 才刷新 registry latest，并由 lockfile 固化解析版本：主 Prompt 使用 OpenTUI 原生 Textarea，命令/Provider Dialog 使用同一 Renderer/focus/key event，Home/Transcript/Prompt/快捷栏使用 Flexbox 响应式布局；旧 Pi TUI 仅暂留 Workspace Trust 与纯回归兼容，不再承担主工作台。
