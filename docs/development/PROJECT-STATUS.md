...
- `REPAIR-PROMPTS #18` 当前**验证中**：Active OpenTUI 原生 Textarea 已把单独 `/` 接回与 Ctrl+P/Ctrl+K 同源的 `commandPaletteOptions() + runCommand()`；命令不是假命令。额外修正可见 `/vivid` shortcut 与内部 `visual` action 的别名分叉，直接 `/vivid` 与面板选择都执行同一真实动作。OpenTUI 合同 38/38、legacy TUI 29/29 PASS；待 Windows Terminal 实机确认输入 `/` 立即弹面板、搜索/执行/Esc 返焦。
- `REPAIR-PROMPTS #17` 当前**验证中**：full-width/right-aligned user band 结构不变，只把背景从偏亮 `#bdbdbd` 下调为克制深灰 `#2d2d30`，文字改为 `#e2e2e2`；待用户 Windows Terminal 目视确认“只有一点白”的亮度。
...
- `REPAIR-PROMPTS #15` 当前**验证中**：Windows `[10]` 对明确 transport/network reset 做有限重试，HTTPS 默认链路仍失败时仅单次命令使用 HTTP/1.1 fallback，不改 Git global/local config；fetch 成功后直接本地 `rebase --autostash origin/main`，避免 `pull` 二次 fetch。Windows Gate/静态合同通过；真实 Git for Windows `curl 56` 恢复待另一台电脑复验。
- `REPAIR-PROMPTS #14` 当前**验证中**：空会话 `turnCount=0` 的 Session detail 已收口为一行最小状态，canonical billing label 左对齐、permission 右对齐；cache hit、tokens、compaction、turn count、cost 等只在第 1 轮真实对话后出现。已有会话仍保持 #12 的完整 metrics 直显与窄屏只换行合同。定向 formatter/static 43/43 PASS、Runtime updater 2/2 PASS、修改文件 TS/TSX 语法转译 PASS、Naming / Architecture / Distribution / Comments / Documentation / AI Context / Version / Windows / Repository 9/9 Gate PASS；等待 Windows Terminal 目视确认 Home 状态栏简洁度与左右对齐。
- `REPAIR-PROMPTS #13` 当前**验证中**：在不改变 #12 整行 user band、Prompt 垂直节奏与 Metrics 全量直显的前提下，继续把用户消息微调为更柔和的浅灰白底，并恢复用户消息在整行 band 内的右对齐显示。当前只修改 `theme.ts` 与 `transcript-viewport.tsx`，等待用户 Windows Terminal 目视验收“不要那么白 + 改回右对齐”。
- `REPAIR-PROMPTS #12` 当前**验证中**：用户消息由 #11 的右侧小块改为 Transcript content column 内 Pi 风格 full-width 浅色 band，左右/上下均有内边距；Session detail 不再按宽度裁字段，`sessionStatusRows()` 只负责按 Terminal 列宽把完整 canonical metrics 稳定分成 1~N 行；Home workbench 通过 Flex bottom padding 轻微上提，并增加 detail 上下留白。纯 formatter 8/8、OpenTUI 静态/快捷键 35/35、Runtime updater 2/2、TS/TSX 语法转译与 9 项 Gate 已通过；Source Manifest 242 files / 候选 ZIP 243 entries 独立解压一致性为 0 差异，并从解压树复跑同组可执行验证通过；真实 Renderable layout 测试已加入，但当前沙箱缺少 Workspace `node_modules/@opentui/core` 且 registry DNS 不可用，不能冒充已执行，仍待 Windows Terminal 最终视觉 E2E。
