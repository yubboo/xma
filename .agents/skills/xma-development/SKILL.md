...
- Windows Bootstrap 的 native 命令输出必须保持实时可见：任何会执行 `pnpm/rustup/cargo/npm/winget` 的动作函数不得被当作业务返回值捕获；这些命令必须直接继承当前 Windows Terminal stdout/stderr，禁止 `Out-Host` / `ForEach-Object` 等 PowerShell pipeline 转码。特别是 `pnpm install` 必须直接继承当前 Windows Terminal 的 stdout/stderr，禁止接 `Out-Host`、`ForEach-Object` 或其他 PowerShell pipeline；否则会把 pnpm 的 `\r` 同行进度刷新拆成逐行输出，并可能造成 Unicode/ANSI 乱码。动作函数保持 void，`Get-*`/状态读取函数才返回对象。
