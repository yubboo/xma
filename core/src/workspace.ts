 ...
 * 关联模块：runtime.ts、session/contract.ts、context.ts、tool/router.ts、plugins/tools/native.ts、未来 Workspace Store/App Protocol。
...
import type { ToolPolicyRequest, ToolSecurityGuard } from './tool/policy.ts'
...
/**
 * Tool Pipeline 的 Workspace 单调 Guard。只有 Tool 明确声明 workspaceAccess 时才检查；无声明工具保持原行为。
 * Guard 只做 TypeScript policy 层拒绝，Native 工具仍必须在 Rust Kernel 里按 roots/capability 再次强制。
 */
