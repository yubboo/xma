...
test('OpenTUI keeps raw reasoning out of transcript, shows thinking status, and projects first answer delta immediately', () => {
  ...
  assert.match(source, /<text fg=\{COLOR\.orange\}><strong>Xiaoyu<\/strong><\/text>/)
  assert.match(source, /<text fg=\{COLOR\.faint\}> · 正在思考<\/text>/)
  assert.match(source, /if \(item\.role === 'reasoning'\) return <><\/>/)
...
test('OpenTUI turns expose a live elapsed-time activity row with public tool actions only', () => {
  ...
  assert.match(source, /function RunActivityRow/)
  assert.match(contract, /startedAtMs: number/)
  assert.match(contract, /outcome: 'running' \| 'completed' \| 'cancelled' \| 'failed'/)
  assert.match(source, /props\.summary\.outcome === 'running'[\s\S]{0,180}props\.nowMs - props\.summary\.startedAtMs/)
  assert.match(source, /`思考了 \${formatRunElapsed\(elapsedMs\(\)\)}/)
  ...
  assert.match(source, /<RunActivityRow summary=\{item\.activity\} nowMs=\{clock\(\)\}/)
  assert.match(source, /props\.summary\.expanded \? '▾' : '▸'/)
  assert.match(source, /onMouseUp=\{event => \{ event\.stopPropagation\(\); props\.onToggle\(\) \}\}/)
  ...
  assert.match(source, /if \(item\.role === 'activity' && item\.activity\)/)
  assert.match(source, /<RunActivityRow summary=\{item\.activity\}/)
...
test('OpenTUI transcript is bottom anchored, keeps user messages right aligned, and preserves manual scrollback', () => {
  ...
  assert.doesNotMatch(source, /Xiaoyu · 思考/)
  assert.match(transcriptSource, /<text fg=\{COLOR\.orange\}><strong>Xiaoyu<\/strong><\/text>\s*<text fg=\{COLOR\.faint\}> · 正在思考<\/text>/)
  assert.match(transcriptSource, /<box width="100%" flexDirection="column">\s*<text fg=\{meta\.color\}><strong>\{meta\.label\}<\/strong><\/text>\s*<text fg=\{COLOR\.text\}>\{item\.text\}<\/text>/)
