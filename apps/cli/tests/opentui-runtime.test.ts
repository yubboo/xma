...
test('OpenTUI approval is explicit Yes/No and Plan handoff is durable before Build execution', () => {
  ...
  assert.match(main, /const planReady = pendingPlanReady as PlanReadyProposal \| undefined/)
  assert.match(main, /result\.status === 'completed' && planReady/)
  assert.match(main, /session\.retainPlan\(result\.turnId, planReady\.plan\)/)
...
test('Active OpenTUI slash prompt reuses the centered ListDialog command palette instead of a second inline UI', () => {
  ...
  assert.match(app, /\.\.\.\(state\.initialQuery !== undefined \? \{ initialQuery: state\.initialQuery \} : \{\}\)/)
  assert.match(app, /\.\.\.\(state\.searchMode !== undefined \? \{ searchMode: state\.searchMode \} : \{\}\)/)
