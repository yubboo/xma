...
test('Active OpenTUI source uses native textarea focus and never reintroduces legacy manual cursor control', () => {
  ...
  assert.match(promptDock, /cursorStyle=\{\{ style: 'block', blinking: true \}\}/)
...
test('OpenTUI keeps raw reasoning out of transcript, shows thinking status, and projects first answer delta immediately', () => {
  ...
  assert.match(source, /if \(event\.type === 'reasoning-delta'\) \{[\s\S]{0,620}recordRunActivity\(\{ kind: 'status', text: '模型思考与规划' \}\)[\s\S]{0,300}setActivity\('thinking'\)[\s\S]{0,160}return/)
  ...
  assert.match(transcriptView, /<text fg=\{COLOR\.faint\}> · 正在思考<\/text>/)
...
test('OpenTUI turns expose a live elapsed-time activity row with public tool actions only', () => {
  ...
  assert.match(transcriptView, /props\.summary\.outcome === 'running'[\s\S]{0,180}props\.nowMs - props\.summary\.startedAtMs/)
  assert.match(transcriptView, /`思考了 \${formatRunElapsed\(elapsedMs\(\)\)}/)
  ...
  assert.match(transcriptView, /onMouseUp=\{event => \{ event\.stopPropagation\(\); props\.onToggle\(\) \}\}/)
  ...
  assert.match(source, /recordRunActivity\(\{ kind: 'status', text: '开始生成最终回复' \}\)/)
...
test('OpenTUI transcript owns a real scroll range, keeps user messages right aligned, and preserves manual scrollback', () => {
  const source = readFileSync('apps/cli/opentui-runtime/app.tsx', 'utf8')
  ...
  assert.match(transcriptSource, /<text fg=\{COLOR\.orange\}><strong>Xiaoyu<\/strong><\/text>\s*<text fg=\{COLOR\.faint\}> · 正在思考<\/text>/)
  ...
  assert.match(readFileSync('apps/cli/opentui-runtime/ui/prompt-dock.tsx', 'utf8'), /<box width=\{props\.width\} flexShrink=\{0\}/)
})
