...
test('Active OpenTUI slash prompt uses first-letter discovery and ghost completion without stealing Tab mode switching', () => {
  const app = readFileSync('apps/cli/opentui-runtime/app.tsx', 'utf8')
  const promptDock = readFileSync('apps/cli/opentui-runtime/ui/prompt-dock.tsx', 'utf8')
  const tui = readFileSync('apps/cli/src/tui.ts', 'utf8')
  assert.doesNotMatch(promptDock, /onOpenCommandPalette/)
  assert.match(promptDock, /slashDiscoveryActive = \(\) => slashInput\(\)\.length > 1/)
  assert.match(promptDock, /slashCommandSuggestions\(slashInput\(\)\)/)
  assert.match(promptDock, /slashCommandCompletionSuffix\(slashInput\(\), selectedSlashSuggestion\(\)\)/)
  assert.match(promptDock, /position="absolute"/)
  assert.match(promptDock, /left=\{slashInput\(\)\.length\}/)
  assert.match(promptDock, /textColor=\{slashDiscoveryActive\(\) \? COLOR\.orange : COLOR\.text\}/)
  assert.match(promptDock, /slashCandidatePrefix/)
  assert.match(promptDock, /slashCandidateSuffix/)
  ...
  assert.match(promptDock, /无匹配命令 · Ctrl\+P 查看全部命令/)
  assert.match(tui, /normalized\.length <= 1/)
