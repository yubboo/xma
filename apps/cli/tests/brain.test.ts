...
test('DeepSeek catalog creates a real official provider profile template without persisting Secret values', () => {
  ...
  assert.deepEqual([...DEEPSEEK_CURRENT_MODELS], ['deepseek-flash', 'deepseek-v4-pro'])
  assert.equal(DEEPSEEK_DEPRECATED_MODEL_IDS.has('deepseek-v4-flash'), true)
  assert.equal(DEEPSEEK_DEPRECATED_MODEL_IDS.has('deepseek-v4-flash-vision-exp'), true)
